...
*************************
DEPRECATED AS OF 04-SEP-2023: The reason of deprecation is that this process is no longer used for the Fleming-Viot estimator
of the blocking probability, as it was replaced by the FV simulation process carried out by the simulators.queues.SimulatorQueue class.
We still leave the code handy because its implementation and test is quite long (6000 lines!) and because it has a value in its own right,
for instance in what concerns the trajectorial treatment of the Markov process, which can be directly generalized to non-Markov processes
as it does not require the Markov assumption of lack of memory of what happened before the current state of a system.
*************************
"""
