...
def plot(plotting_func,
    ...
    dict_params: (opt) dict
        ...
        - 'pointlabels': Name(s) of the variables in `df` to use as point labels in each subplot.
    ...
    dict_options: (opt) dict
        Dictionary of plotting options.
        default: {  'axis': {   # Fontsize to use for ALL texts: title, axis labels, axis tick marks, legend
                                'fontsize': 13,
                                'limits': {'xmin': None, 'xmax': None,
                                           'ymin': None, 'ymax': None,
        ...
        These default values can be checked by running function `plotting.default_plot_options()`.
    ...
    fontsize = dict_options_local['axis']['fontsize']
    ...
    showmeans_default = showmeans = False
    showmedians_default = showmedians = True
    if dict_params.get('violin') is not None:
        showmeans = dict_params['violin'].get('showmeans', showmeans_default)
        showmedians = dict_params['violin'].get('showmedians', showmedians_default)
    ...
    for k, ax in enumerate(axes1D):
        ...
        elif re.search("plot_violin", plotting_func.__name__):
            # Violin plot
            legend_objects, legend_texts = plotting_func(ax, df, x, y,
                                                         showmeans=showmeans,
                                                         showmedians=showmedians,
                                                         dict_options=dict_options_k)
        else:
        ...
        if labelvar is not None and labelvar != "":
            ...
            for xx, yy, label in zip(df[x] * dict_options_local['multipliers']['x'], df[y] * dict_options_local['multipliers']['y'], df[labelvar]):
                ax.text(xx, yy, label, fontsize=fontsize)
        if yref is not None:
        ...
        ax.legend(legend_objects, legend_texts, fontsize=fontsize)
    ...
    for k, ax in enumerate(axes1D):
        ...
        # Axis tickmarks (fontsize)
        # Ref: https://stackoverflow.com/questions/34001751/how-to-increase-reduce-the-fontsize-of-x-and-y-tick-labels
        ...
        if dict_options_local['axis']['tickmarks']['x2'][k] is not None:
            ...
            ax2.set_xticklabels(["{:,.0f}".format(x2) for x2 in x2_values], rotation=90, fontsize=fontsize)
            ax2.set_xlabel(dict_options_local['labels'].get('x2', "")[k], fontsize=fontsize)
    ...
    if dict_options_local['showtitle'] and dict_options_local['texts']['title'] is not None:
        plt.suptitle(dict_options_local['texts']['title'], fontsize=fontsize)
...
def plot_splines(ax, df, x, y, w=None, s=None, dict_options=dict()):
    ...
    dict_options: (opt) dict
        ...
        Default values can be seen by running function `plotting.default_plot_options()`.
