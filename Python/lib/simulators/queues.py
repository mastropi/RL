...
from Python.lib.estimators import DEBUG_ESTIMATORS, estimate_expected_cycle_time, estimate_expected_stopping_time_in_cycle, estimate_state_distribution_based_on_observed_times, \
    estimate_stationary_probabilities_simple_average
...
def estimate_blocking_mc(env, agent, dict_params_simul, dict_params_info, start_queue_state=None):
    ...
    t, time_last_event, n_return_cycles, time_last_return, return_times = \
            run_simulation_mc(env, agent, dict_params_info.get('t_learn', 1), start_state, dict_params_simul['T'],
...
def run_simulation_mc(env, agent, t_learn, start_state, t_sim_max,
    ...
    """
    Runs the continuous-time simulation using Monte-Carlo while optionally keeping track of entry times to an absorption set A and
    ...
    t_sim_max: int
        ...
        the absorption set A) (for further details, see calculations in my green small notebook written on 06-Nov-2022).

    absorption_set: (opt) SetOfStates
        Absorption set whose boundary defines the entry events whose times are tracked.
        default: None, in which case track_return_cycles and track_absorptions must be False
    ...
    if track_return_cycles or track_absorptions:
    ...
    while not done:
        ...
        if event == Event.BIRTH:
            ...
            # Note that the state of the environment has been updated by the manage_job_arrival() function, that's why env.getQueueState() returns the value of `next_state` above
            assert env.getQueueStateFromState(next_state) == env.getQueueState()
            if (track_return_cycles or track_absorptions) and \
                is_state_in_boundary_of_setofstates(env.getQueueStateFromState(state), absorption_set) and is_state_in_boundary_of_setofstates(env.getQueueState(), activation_set):
            ...
            assert env.getQueueStateFromState(next_state) == env.getQueueState()
            if (track_return_cycles or track_absorptions) and \
                is_state_in_boundary_of_setofstates(env.getQueueStateFromState(state), activation_set) and is_state_in_boundary_of_setofstates(env.getQueueState(), absorption_set):
...
def estimate_blocking_mcp(env, agent, dict_params_simul, dict_params_info, start_queue_state=None):
    ...
    For more details of the estimation process, see the documentation for estimate_stationary_probabilities_simple_average().
    ...
    # Set the simulation seed
    # Note that, although the seed is set by the queue environment (via its set_seed() method which is called by run_simulation_and_estimation_mcp() below)
    # we also need to set the seed here because, before calling run_simulation_mc(), we choose below the initial queue state
    # on which the simulation starts randomly, among all possible states in the absorption set.
    np.random.seed(dict_params_simul['seed'])

    # Reset the environment and the learner of the value functions
    # IMPORTANT: We should NOT reset the learner of the policy because this function could be called as part of a
    # policy learning process! Note that the learner of the value function V MUST be reset regardless of this because
    # at this point a new queue simulation starts and at that point the value function should be learned anew.
    env.reset()
    ...
    # with the Fleming-Viot method where particles are reactivated when they reach a state in the absorption set.
    if start_queue_state is None:
        assert "absorption_set" in dict_params_simul.keys(), "Entry 'absorption_set' should be present in the `dict_params_simul` dictionary when `start_queue_state` is None"
        start_queue_state = dict_params_simul['absorption_set'].random_choice()
    start_state = (start_queue_state, None)
    # -- Parse input parameters

    probas_stationary_blocking, probas_stationary_blocking_first_hit, n_events, time_last_event = \
        run_simulation_and_estimation_mcp(env, agent, dict_params_info.get('t_learn', 1), start_state, dict_params_simul['T'], dict_params_simul['N'],
                                          seed=dict_params_simul['seed'],
                                          verbose=dict_params_info.get('verbose', False), verbose_period=dict_params_info.get('verbose_period', 1))
...
@measure_exec_time
def run_simulation_and_estimation_mcp(env, agent, t_learn, start_state, t_sim_max, N, burnin_time=0.0, seed=None, verbose=False, verbose_period=1):
    """
    ...
    N: int
        Number of particles (queues) to run in parallel (although, to reduce memory storage, they are run one after the other).
    ...
    seed: (opt) int
        Seed to use as BASE seed for the simulation of each particle. The actual seed is defined by this seed + a value that depends on th particle index.
        default: None
    ...
    Return: tuple
    - probas_stationary_blocking: the blocking probability estimated as the average time spent at the blocking states by each particle averaged across particles.
    - probas_stationary_blocking_first_hit: the blocking probability estimated as the proportion of particles hitting the blocking states
                                            (an approximation of the above which should work well for very small probabilities).
    - n_events: the total number of time steps run over all particls (integer-valued).
    - max_time_last_event: maximum continuous time observed for all particles in the parallel system.
    """
    #-- Parse input parameters
    # Maximum number of steps to run for each particle
    t_max_per_particle = t_sim_max // N

    # Seed that is at the base of the seeds used in each particle's simulation
    seed_base = seed

    # Initialize the dictionary that will store the stationary probabilities estimated by each particle and the probability of hitting each state of interest
    # The state of interest is the key index of the respective dictionary
    probas_stationary = dict()
    probas_first_hit = dict()

    # Get the states or buffer sizes on which the stationary probability should be estimated
    states_or_buffer_sizes_of_interest = get_blocking_states_or_buffer_sizes(env, agent)
    for s in states_or_buffer_sizes_of_interest:
        probas_stationary[s] = 0.0
        probas_first_hit[s] = 0.0

    # Run the Monte-Carlo simulation separately for each particle and update the estimated stationary probability for each state of interest
    max_time_last_event = 0.0
    n_events = 0
    for i in range(N):
        # Reset the learner stored in the agent so that all trajectories are reset
        agent.getLearnerV().reset()

        t, time_last_event = run_simulation_mc( env, agent, t_learn, start_state, t_max_per_particle,
                                                seed=None if seed_base is None else seed_base + i*1313, verbose=verbose, verbose_period=verbose_period)
        max_time_last_event = max(max_time_last_event, time_last_event)
        n_events += t

        # DONE
        if show_messages(verbose, verbose_period, t_learn):
            print("[MCP] ==> Simulation for particle #{} ENDS at time t={}, absolute (continuous) time = {:.3f})" \
                  .format(i+1, t, time_last_event))

        for s in states_or_buffer_sizes_of_interest:
            probas_stationary[s] += estimate_stationary_probabilities_simple_average(env, agent, time_last_event, s, burnin_time=burnin_time)
            if probas_stationary[s] > 0:
                probas_first_hit[s] += 1

    # Compute the probabilities from the cumulative quantities computed during the loop by dividing by the number of particles, as this gives the average of those quantities
    # over the different realizations observed across particles
    for s in states_or_buffer_sizes_of_interest:
        probas_stationary[s] /= N
        probas_first_hit[s] /= N

    return probas_stationary, probas_first_hit, n_events, max_time_last_event
...
def estimate_blocking_fv(envs, agent, dict_params_simul, dict_params_info, use_exit_state_distribution: bool=True):
    ...
    if dict_params_simul['method_survival_probability_estimation'] == SurvivalProbabilityEstimation.FROM_M_CYCLES:
        ...
        t, time_end_simulation, n_cycles_absorption, time_last_absorption, exit_times, absorption_times, dict_exit_states_and_times, survival_times = \
            run_simulation_mc(envs[0], agent, dict_params_info.get('t_learn', 1), (start_queue_state_boundary_A, None),
                              dict_params_simul['T'],
                              absorption_set=dict_params_simul['absorption_set'],
                              activation_set=dict_params_simul['activation_set'],
...
def run_simulation_fv(t_learn, envs, agent, absorption_set: SetOfStates, activation_set: SetOfStates, dist_proba_for_start_state: dict=None,
    ...
    dict_phi = initialize_phi(envs, agent=agent, t=event_times[0])
    ...
    while not done:
        ...
        if event == Event.BIRTH:
            ...
            if is_state_in_boundary_of_setofstates(env.getQueueStateFromState(state), activation_set) and is_state_in_boundary_of_setofstates(envs[idx_particle].getQueueState(), absorption_set):
                ...
                #idx_reactivate = reactivate_particle(envs, idx_particle, states_or_buffer_sizes_of_interest[-1], reactivation_number=reactivation_number)
