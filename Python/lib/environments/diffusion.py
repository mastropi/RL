...
class EnvDiffusion(EnvironmentContinuous):
    ...
    **kwargs: additional parameters needed for the simulation of the process.
    ...
    Parameters could also encode a jump process to create a Levy process, in which case the user should pass a single dictionary parameter, `levy`
    with the following entries:
    - lambda: the intensity of the Poisson process defining the jump times. A Poisson is then drawn with parameter `lambda*dt` to define the number
    of jumps occurring in the discretized interval dt whose amplitude is independently generated and summed, to define the total jump value.
    - fun_dist_jumps: callable defining the probability distribution function to use for the jumps.
    - params_dist_jumps: (opt) dictionary containing the parameters of the `fun_dist_jumps` callable, as well as optional parameters affecting
    the random value generated by the given callable, as follows:
        - 'shift': value by which the random value should be shifted by (default: 0.0)
        - 'factor': factor by which the random value should be multiplied by after shifting (default: 1.0).
        Ex:
        `{'factor': -1.0, 'loc': 1.0, 'scale': 0.2}` when `fun_dist_jumps = np.random.normal` --> r.v. = factor * np.random.normal(loc=loc, scale=scale)
        `{'factor': -1.0, 'shift': 0.8, 'mean': 0.5, 'sigma': 0.2}` when `fun_dist_jumps = np.random.lognormal` --> r.v. = factor * (shift + np.random.lognormal(mean=mean, sigma=sigma))
    """
    ...
    def step(self, action):
        ...
        # Levy process
        if self.dict_params.get('levy') is not None:
            # Add a Levy jump
            _params_levy = self.dict_params['levy']
            if not isinstance(_params_levy, dict):
                raise ValueError(f"Input parameter `levy` must be a dictionary: {_params_levy}")
            if _params_levy.get('lambda') is None:
                raise ValueError(f"Input dictionary `levy` must contain the entry `lambda` with the intensity of the jump process: {_params_levy}")
            if _params_levy.get('fun_dist_jumps') is None:
                raise ValueError(f"Input dictionary `levy` must contain the entry `fun_dist_jumps` with the callable defining the probability distribution on which each jump is computed: {_params_levy}")
            # Parse the jumps definition
            _lambda = _params_levy['lambda']
            _fun_dist_jumps = _params_levy['fun_dist_jumps']
            _params_dist_jumps = _params_levy.get('params_dist_jumps', dict({'factor': 1.0, 'shift': 0.0}))
            _params_for_dist = keep_dict_params_defined_in_function(_params_dist_jumps, _fun_dist_jumps)

            # Sample number of jumps occurring in the dt interval
            _n_jumps = np.random.poisson(_lambda*self.dt)

            # Sample magnitude of each jump and sum them
            _jump_value = 0.0
            for _ in range(_n_jumps):
                _jump_value += _params_dist_jumps['factor'] * (_params_dist_jumps['shift'] + _fun_dist_jumps(**_params_for_dist))

            # Update the next state with the observed jumps
            next_state += _jump_value
            info['n_jumps'] = _n_jumps
            info['jump_value'] = _jump_value
