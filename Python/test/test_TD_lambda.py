...
class Test_TD_Lambda_GW2D(unittest.TestCase, test_utils.EpisodeSimulation):
    ...
    def setUpClass(cls):    # cls is the class, in this case, class 'Test_TD_Lambda'
        ...
        # Environment: 2D gridworld
        cls.ny = 12
        cls.nx = 12
        cls.nS = cls.ny * cls.nx
        cls.fontsize = np.sqrt(16/(cls.ny*cls.nx)) * 16 # Fontsize is a 1D dimension => we make it proportional to 16 by applying the sqrt to the proportion 16/Area
    ...
    #-------------------------------------- DATA FOR TESTS ------------------------------------
    # Case number, description, expected value, parameters
    todo_data_test_random_walk = lambda: (
            ( 1, 'TD(0), no alpha adjustment',
                 [-1.000000, -0.987504, -0.941069, -0.900173, -0.805758, -0.613337, -0.407574,
                  -0.298142, -0.221312, -0.149854, -0.051759, 0.001865, 0.052824, 0.128984,
                  0.217968, 0.297497, 0.478097, 0.680931, 0.810098, 0.961462, 1.000000],
    ...
    #------------------------------------------- TESTS ----------------------------------------
    @data_provider(todo_data_test_random_walk)
    def no_test_random_walk(self, casenum, desc, expected, params_alpha_gamma_lambda,
                                                        adjust_alpha, adjust_alpha_by_episode, alpha_min):
        print("\nTesting " + self.id())
    ...
    def test_random_walk_onecase(self):
        ...
        print("Environment ends at state {}:".format(self.env.getState()))
        self.env._render()
        observed = np.asarray( agent_rw_tdlambda.getLearner().getV().getValues() ).reshape(self.ny, self.nx)
        print("\nobserved: ")
        print(observed)
