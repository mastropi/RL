...
class Test_Class_nn_backprop(unittest.TestCase):
    ...
    @staticmethod
    def _assert_parameter_values(nn_model, expected_values, atol=1E-4):
        assert len(list(nn_model.parameters())) == (len(nn_model.hidden_layers) + 1) * 2, "The number of neural network weights must be equal to twice the number of hidden and output layers ('twice' because there biases are stored as a separate tensor)"
    ...
    def test_model_architecture(self):
        ...
        self._assert_parameter_values(nn_model, expected_params)
